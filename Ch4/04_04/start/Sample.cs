    ...
    class Sample
    {
        ...
        static Tuple<double, double>[] initalizePatients(int size) {
            ...
            while(i < size) {
                ...
                // ERROR
                double inches = Math.Round(nextDouble(rnd, 0, 11), 2); // random inch value between 0 and 11
        ...
        static double[] calculate_BMI_Collection(Tuple<double, double>[] patients) {
            ...
            for(int i = 0; i < bmi_collection.Length; i++) {
                bmi_collection[i] = Math.Round(calculate_BMI(patients[0].Item1, patients[0].Item2), 2);
            }
